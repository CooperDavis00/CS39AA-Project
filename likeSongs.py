...
for index, row in df.iterrows():
    ...
    artist_names = row['Artist Name(s)'].split(', ')  # Assuming artists are separated by a comma and space
    track_uri = row['Track URI']
    track_id = row['track_id']

    # Get track details, including artists, using the Spotify API
    try:
...
updated_df = pd.DataFrame(updated_rows, columns=['Track Name', 'Artist Name(s)', 'Genre'])
